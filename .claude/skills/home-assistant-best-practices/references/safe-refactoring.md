...
To inspect current members of a specific group, initiate an Options Flow and read `suggested_value` in the returned `data_schema.entities` field:
...
1. Initiate a new fix flow. Read the current option values from `suggested_value`. Note the `flow_id`:
...
**Entity registry renames only update the Entity Registry.** Integrations that collect entity_ids during their setup flow store them in the Config Entry — not in YAML and not in the Entity Registry.

**Affected integrations and storage fields:**

| Integration | Storage field | Fields containing entity_ids |
|---|---|---|
| **Better Thermostat** | `data` (not accessible via REST) | `temperature_sensor`, `humidity_sensor`, `outdoor_sensor`, `window_sensors` |
...
For integrations that store entity_ids in `options`: use the Options Flow (see Config-Entry-Groups section above for the full pattern).

For integrations that store entity_ids in `data` (Better Thermostat): no API-based fix path exists. Document this limitation to the user before proceeding.
