...
| UniFi Network Application | UniFi OS Server (VM), site `8a3bp31m` ("Wake Forest") | `192.168.1.58:11443` | API-key auth. The `unifi-network` MCP plugin was uninstalled 2026-08-20 (403'd against this server on both username/password and API-key paths) — use direct API-key HTTP calls (`X-API-KEY` header) against `/proxy/network/integration/v1/sites/{site}/...` instead. Old classic controller (`unifi-controller-reborn`, `192.168.1.55:8443`) fully decommissioned 2026-08-18. **Correct call shape (fixed 2026-08-28, was 401ing before):** use env var `UNIFI_NETWORK_API_KEY_CONTAINER` (not `UNIFI_NETWORK_API_KEY`, which is stale), and the `{site}` path segment must be the real site **UUID** (`3281e015-15b8-3fed-bfef-41e2e5fad995` for "Wake Forest"), not the short internal reference `8a3bp31m` — fetch `GET /proxy/network/integration/v1/sites` first if the UUID is ever in doubt, since it lists both `id` (UUID) and `internalReference` (short name) per site. |
...
**Skills** (`.claude/skills/*/SKILL.md`) encode HA/UniFi domain procedures the hooks can't (or shouldn't) enforce mechanically — e.g. `ha-yaml-linter` (full lint ruleset), `ha-dashboard-yaml` (pinned card-stack versions/syntax), `ha-audit` (phased audit workflow), `crowdsec-bouncer` (two distinct, unrelated root causes for the same symptom — diagnose before fixing), `scrub` (secret/PII redaction for anything about to leave this workspace), `eng-task` (Steps 3–5 of the Notion "AI-Assisted Infrastructure Workflow": load a task's context packet, execute in scope, validate/review, closure-sync back to Notion; `/eng-task new: <objective>` files a task instead), `rotate` (standardized credential rotation).
...
- `rotate-homepage-keys.sh` — **runs on the Unraid host** (needs `docker` + `/mnt/cache/appdata` paths). Rotates the 7 machine-generatable Homepage widget API keys exposed 2026-08-28 (sonarr/radarr/prowlarr/bazarr/sabnzbd/tautulli/gluetun): generate → back up config → stop → rewrite key in place → restart → verify against the service's own API, with automatic backup-restore on verify failure. New values go only to an `0600` env file; a manual propagation checklist for downstream consumers prints at the end. Does **not** touch Unraid/UniFi/Plex (Phase C, no mint API) or edit consumer DBs. `--dry-run` / `--only SERVICE` / `--restore STAMP`. Edit-function unit tests: `scripts/test-rotate-homepage-keys.sh`.
